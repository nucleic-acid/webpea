...
The goal of webpea is to provide easy functions to save a plot in the [WebP image file format](https://en.wikipedia.org/wiki/WebP).
...
`webpea::webpea()` serves as drop-in replacement of `ggplot2::ggsave()`. First the plot is saved as a temporary PNG file, so all `ggplot2::ggsave()` parameters can be used as usual. Secondly the temporary PNG file is converted to WebP via the {magick} package.
...
All parameters for `ggplot2::ggsave()` are valid and can be specified in `webpea()`. You can also explicitly specify the plot to be saved. Additionally, you can specify the quality as percentage (defaults to 75 in the {magick} package, if not specified otherwise):
